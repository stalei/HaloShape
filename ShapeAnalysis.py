...
def CompareEllipsoids(ell1,ell2):
    AreSame=False;
    dax=np.abs(ell1.axis/ell2.axis)
    dori=np.abs(ell1.orientations/ell2.orientations)
    #uax=np.ones(3)
    #uori=np.ones(3,3)
...
def _GetShape(halo,ds,Rvir):
    ...
    for i in range(0,len(Rbins)):
        ...
        while(not(convergence) and iteration<iteLim):
            ...
            #newell(axisNew,orientationNew)
            if i>0:
                convergence=False#CompareEllipsoids(ell,newell)
                if(not convergence):
                    #iteration+=1
                    axis=axisNew
